...
class ConnectionManager:
    ...
    def connect_serial(self, port: str, baudrate: int = 9600, timeout: int = 1) -> bool:
    ...
    def connect_ssh(self, hostname: str, username: str, password: str, port: int = 22) -> bool:
        """Connect via SSH"""
        try:
            logger.info(f"Connecting to {hostname}:{port} via SSH as {username}")
            ...
            # Test connection
            stdin, stdout, stderr = self.connection.exec_command('echo "Connection test"')
    ...
    def disconnect(self):
        ...
        if self.connection:
            try:
                ...
                logger.info(f"Disconnected from {self.connection_type} connection")
