...
class CVEManager:
    ...
    def fetch_cves_for_vendor(
        self, vendor: str, product: str = None, max_results: int = 100
    ) -> List[Dict]:
        ...
        try:
            ...
            response = requests.get(
                self.nvd_base_url, params=params, headers=headers, timeout=30
            )
    ...
    def get_database_stats(self) -> Dict:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute(
                "SELECT COUNT(DISTINCT vendor) FROM vendor_products"
            )
            total_vendors = cursor.fetchone()[0]

            cursor.execute(
                "SELECT COUNT(DISTINCT product) FROM vendor_products"
            )
