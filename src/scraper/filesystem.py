...
class FileSystemScraper:
    ...
    def explore_directory(
        self, path: str, max_depth: int = 2, current_depth: int = 0
    ):
        ...
        for cmd in commands:
            ...
            if (
                listing
                and "No such file" not in listing
                and "not found" not in listing.lower()
            ):
                break
    ...
    def check_default_configurations(self):
        ...
        for config_file in config_files[
            ...
            content = self.connection.send_command(
                f"cat {config_file['path']}"
            )
            if content:
    ...
    def check_running_services(self):
        ...
        commands = ["ps aux", "ps -ef", "ps", "netstat -an", "netstat -l"]
    ...
    def get_file_content(
        self, filepath: str, max_size: int = 1000
    ) -> Optional[str]:
